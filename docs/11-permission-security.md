...
1. 工具结果隔离：工具输出明确标着 `tool_result`，模型能区分用户指令和工具输出
2. Bash 验证器：命令替换（`$()`、反引号）会被检测并标记
3. 路径约束：防止文件内容注入指令后再执行文件外操作
...
2. 安全问题：如果剥离了危险的环境变量，`LD_PRELOAD=/evil.so npm test` 就会被当成安全的 `npm test` 放行
...
以下变量绝不会被剥离——它们留在命令里参与权限匹配，因为它们能影响代码执行：
